...
兼容尚未读取共享租约的旧版回收方：借用时还在资源锁内创建或沿用 `.worktree-keep`，
旧版删除与池化复用均已识别该哨兵。用户已有或后来修改/替换的标记不会被删除；Host 自建
标记只在最后一个共享借用者结束、文件身份与内容仍匹配时移除。发布失败则不开始源目录
I/O；清理失败通过共享 `.release` 回执重试，异常退出留下的租约继续保护旧版回收路径。
