...
const state = vi.hoisted(() => ({
  ...
  ingest: vi.fn(), query: vi.fn(),
}));
vi.mock('electron', () => ({ app: { getPath: () => state.root } }));
vi.mock('../../localDb/client/current.js', () => ({ getCurrentDbClientSnapshot: () => state.snapshot }));
vi.mock('../../cindy-media/refCompensationJournal.js', () => ({ captureMediaRefCompensationScope: () => ({ assertStillValid: () => { if (state.invalidOwner) throw new Error('owner changed'); } }) }));
vi.mock('../../cindy-media/ingest.js', () => ({ ingestMedia: (...args: unknown[]) => state.ingest(...args) }));
vi.mock('../../cindy-media/blobStore.js', () => ({ resolveSafe: () => ({ absPath: path.join(state.root, 'saved.png') }) }));
...
it('uses durable media ingestion and session refs for recognized media', async () => {
  ...
  expect(state.ingest.mock.calls[0][0]).toMatchObject({ mimeType: 'image/png', isCache: false, refs: [{ refId: 'test-session', refKind: 'session-attachment' }] });
