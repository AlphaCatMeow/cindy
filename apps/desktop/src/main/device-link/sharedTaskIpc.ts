...
export function registerSharedTaskIpc(available: () => boolean): void {
  ipcMain.handle(SHARED_TASK_HOST_CHANNEL, async (event, raw: unknown) => {
    ...
    return executeSharedTaskHostCommand(raw, { available, host: requireSharedTaskHost });
  });
  ipcMain.handle(SHARED_TASK_ACCOUNT_CHANNEL, async (event, raw: unknown) => {
    ...
    if (!available()) throwIpcError('UNSUPPORTED_CAPABILITY', 'SharedTask mode requires updated clients and server');
